...
for x in range(0, timestep_sim):
    ...
    if timestep_sim >1:
        ExportView(path_paraview + str(statefile) + '_timestep' + str(x+1) + '_time' + str(tsteps[x]) + '_' + export_format_paraview, view=lineChartView1)
    else:
        ExportView(path_paraview + str(statefile) + '_timestep1_' + export_format_paraview, view=lineChartView1)        
...
if metadata == 'y':
    for mydata in glob.glob(mypath + '*.csv'):
        shutil.copy(mydata, os.getcwd() + '/' + 'process/')
    for mydata in glob.glob(mypath + '*.txt'):
        shutil.copy(mydata, os.getcwd() + '/' + 'process/')
